...
def apiauth(vip='helios.cohesity.com', username='helios', domain='local', password=None, newPassword=None,
    ...
    if pwd is not None:
        ...
        pwd = __getpassword(vip=vip, username=username, password=password, domain=domain, useApiKey=useApiKey,
                            helios=helios, updatepw=updatepw, prompt=prompt)
    if pwd is None:
    ...
    if entraId is True and vip.lower() in HELIOSENDPOINTS:
        ...
        if directoryId is None:
            directoryId = __getpassword(vip=vip, username=username, password=password, domain=domain,
                                        useApiKey=useApiKey, helios=helios, updatepw=updatepw, prompt=prompt,
                                        directoryId=True)
        if clientId is None:
            clientId = __getpassword(vip=vip, username=username, password=password, domain=domain, useApiKey=useApiKey,
                                     helios=helios, updatepw=updatepw, prompt=prompt, clientId=True)
        if clientId is not None and directoryId is not None and scope is not None:
            token = ProcessOidcToken(username=username, password=pwd, client_id=clientId, tenant_id=directoryId,
                                     scope=scope)
            if token is not None:
                COHESITY_API['HEADER'] = {'accept': 'application/json', 'content-type': 'application/json',
                                          'X-OPEN-ID-AUTHZ-TOKEN': token, 'User-Agent': 'pyhesity/%s' % api_version}
            else:
        ...
        HELIOSCLUSTERS = (
            COHESITY_API['SESSION'].get(URL, headers=COHESITY_API['HEADER'], verify=False, timeout=timeout)).json()
        if HELIOSCLUSTERS is not None and 'message' in HELIOSCLUSTERS:
            ...
            if 'Authentication failed' in HELIOSCLUSTERS['message'] and noretry is False and prompt is not False:
                apiauth(vip=vip, username=username, domain=domain, updatepw=True, prompt=prompt, helios=helios,
        ...
        try:
            ...
            if HELIOSCLUSTERS is not None and 'errorCode' not in HELIOSCLUSTERS:
                ...
                REGIONS = (COHESITY_API['SESSION'].get(URL, headers=COHESITY_API['HEADER'], verify=False,
            ...
            if setpasswd is not None:
                pwd = __getpassword(vip=vip, username=username, password=setpasswd, domain=domain, useApiKey=useApiKey,
                                    helios=helios, updatepw=updatepw, prompt=prompt)
        ...
        if cluster is not None and 'preferences' in cluster:
            ...
            COHESITY_API['AUTHENTICATED'] = False
            if 'API Key does not exist' in COHESITY_API['LAST_ERROR'] or 'StatusUnauthorized' in COHESITY_API[
                'LAST_ERROR'] or 'invalid header value' in COHESITY_API['LAST_ERROR']:
            ...
            if response != '':
                if response.status_code == 201 or response.status_code == 200:
                    ...
                    try:
                        ...
                        user = response.json()
                        if 'user' in user and user['user'] is not None and 'forcePasswordChange' in user['user'] and \
                                user['user']['forcePasswordChange'] is True:
                        ...
                        if changePassword is True:
                            ...
                            response = COHESITY_API['SESSION'].post(url, data=creds, headers=COHESITY_API['HEADER'],
                                                                    verify=False, timeout=timeout)
                            if response != '':
                                if response.status_code == 201 or response.status_code == 200:
                                    COHESITY_API['AUTHENTICATED'] = True
                                    pwd = __getpassword(vip=vip, username=username, password=newPassword, domain=domain,
                                                        useApiKey=useApiKey, helios=helios, updatepw=updatepw,
                                                        prompt=prompt)
                                    setpasswd = None
                    ...
                    if mfaCode is not None or emailMfaCode is True:
                        ...
                        if emailMfaCode is True:
                            ...
                            response = COHESITY_API['SESSION'].post(url, data=None, headers=COHESITY_API['HEADER'],
                                                                    verify=False, timeout=timeout)
                        ...
                        try:
                            ...
                            response = COHESITY_API['SESSION'].post(url, data=mfaCheck, headers=COHESITY_API['HEADER'],
                                                                    verify=False, timeout=timeout)
                    ...
                    COHESITY_API['LAST_ERROR'] = 'OK'
                    if setpasswd is not None:
                        pwd = __getpassword(vip=vip, username=username, password=setpasswd, domain=domain,
                                            useApiKey=useApiKey, helios=helios, updatepw=updatepw, prompt=prompt)
                    if quiet is None:
                        print("Connected!")
                    COHESITY_API['COOKIES'] = COHESITY_API['SESSION'].cookies.get_dict()
                    ...
                    if response.status_code == 403 and 'user does not have the privilege to access ui' in \
                        ...
                        # =============================================================================================================
                        creds = json.dumps({"domain": domain, "password": pwd, "username": username, "otpType": mfaType,
                                            "otpCode": mfaCode})
                        ...
                        try:
                            ...
                            if response != '':
                                if response.status_code == 201:
                                    ...
                                    # try session auth
                                    if response.status_code == 400 and 'access denied' in response.json()[
                                        'message'].lower():
                                        try:
                                            url = COHESITY_API['APIROOTv2'] + 'users/sessions'
                                            creds = json.dumps({"domain": domain, "password": pwd, "username": username,
                                                                "otpType": mfaType.lower(), "otpCode": mfaCode})
                                            if emailMfaCode is True:
                                                creds = json.dumps(
                                                    {"domain": domain, "password": pwd, "username": username,
                                                     "otpType": 'email', "otpCode": mfaCode})
                                            response = requests.post(url, data=creds, headers=COHESITY_API['HEADER'],
                                                                     verify=False, timeout=timeout)
                                            if response != '':
                                                if response.status_code == 201:
                                        ...
                                        if response.status_code == 400 and 'invalid username' in response.json()[
                                            'message'].lower():
                                            if noretry is False and prompt is not False:
                                                apiauth(vip=vip, username=username, domain=domain, updatepw=True,
                                                        prompt=prompt, helios=helios, useApiKey=useApiKey)
                        except requests.exceptions.RequestException as e:
                            __writelog(e)
                            COHESITY_API['AUTHENTICATED'] = False
                            COHESITY_API['LAST_ERROR'] = e
                    ...
                    if quiet is None:
                        print(COHESITY_API['LAST_ERROR'])
                    if (response.status_code == 400 or response.status_code == 401) and 'invalid username' in \
                            response.json()['message'].lower():
                        if noretry is False and prompt is not False and setpasswd is None:
                            apiauth(vip=vip, username=username, domain=domain, updatepw=True, prompt=prompt,
...
def api(method, uri, data=None, quiet=None, mcm=None, mcmv2=None, v=1, reportingv2=None, context=None, timeout=300):
    ...
    if method in APIMETHODS:
        try:
            if method == 'get':
                response = COHESITY_API['SESSION'].get(url, headers=THISCONTEXT['HEADER'], verify=False,
                                                       timeout=timeout)
            if method == 'post':
                response = COHESITY_API['SESSION'].post(url, headers=THISCONTEXT['HEADER'], json=data, verify=False,
                                                        timeout=timeout)
            if method == 'put':
                response = COHESITY_API['SESSION'].put(url, headers=THISCONTEXT['HEADER'], json=data, verify=False,
                                                       timeout=timeout)
            if method == 'delete':
                response = COHESITY_API['SESSION'].delete(url, headers=THISCONTEXT['HEADER'], json=data, verify=False,
                                                          timeout=timeout)
            COHESITY_API['LAST_ERROR'] = 'OK'
        except requests.exceptions.RequestException as e:
            __writelog(e)
...
def __getpassword(vip, username, password, domain, useApiKey=False, helios=False, updatepw=False, prompt=True,
                  directoryId=False, clientId=False):
...
def setpwd(v='helios.cohesity.com', u='helios', d='local', useApiKey=False, helios=False, password=None, entraId=False,
           directoryId=False, clientId=False):
    originalUsername = u
    originalVip = v
...
def fileUpload(uri, fileName, v=1, timeout=300):
    ...
    try:
        ...
        response = COHESITY_API['SESSION'].post(url, files=files, headers=COHESITY_API['HEADER'], verify=False,
                                                timeout=timeout, cookies=COHESITY_API['COOKIES'])
    except Exception as e:
