...
class VMwareClient:
    def __init__(self, config):
        ...
        if not self.password:
            print(Fore.RED + "Error: VMWARE_PASSWORD environment variable not set")
            sys.exit(1)
    ...
    def graceful_shutdown_vm(self, vm, timeout=120):
        if vm.runtime.powerState == vim.VirtualMachinePowerState.poweredOff:
            return  # Already off
        print(Fore.YELLOW + f"Attempting graceful shutdown of VM: {vm.name}")
        if self.shutdown_guest(vm):
            start = time.time()
            while time.time() - start < timeout:
                vm = self.get_vm_by_name(vm.name)
                if vm.runtime.powerState == vim.VirtualMachinePowerState.poweredOff:
                    print(Fore.GREEN + f"VM {vm.name} gracefully shut down.")
                    return
                time.sleep(5)
            print(Fore.RED + f"Graceful shutdown timed out. Forcing power off {vm.name}")
        else:
            print(Fore.YELLOW + f"VMware Tools not available or shutdown failed, forcing power off {vm.name}")

        self.power_off_vm(vm)

    def get_vm_ip(self, vm):
        print(Fore.GREEN + f"Waiting for IP address of VM: {vm.name}")
